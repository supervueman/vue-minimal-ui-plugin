...
<script>
  ...
  export default {
    ...
    data() {
      return {
        count: 1, // Мы не идиоты, просто нужна была поддержка IE11, а этот супер-пупер браузер вызывает функцию selectFiles 2 раза по событию change!!!
      };
    },
    methods: {
      openFileWindow() {
      ...
      selectFiles($event) {
        if (this.count === 1) {
          ...
          Promise.all(this.filePromiseCollect($event.target.files, files)).then(
            () => {
